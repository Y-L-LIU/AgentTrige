...
import json
tree = json.load(open('haigong/test.json'))
cls = TreeAPI(tree)
...
def search_given_children(k, text):
    '''
...
def move_to_kth_child(k):
    '''
    Move to the k-th child of the current node. If you find the contetnt of entered child is different from the child you want\
        , it is possible that you have given a a wrong index, you can try input the given context of the child and this tool can try to find the correct child.
    Args: 
    ...
    if type(k)==str:
        found=0
        for ind,item in enumerate(cls.pos['children']):
            if k in item['text']:
                k=ind
                found=1
                break 
        if not found:
            return f"There are problem with the given index, the feasible index is from 0 to {len(cls.pos['children'])-1} but {k} is given."
    if 0<=k<=len(cls.pos['children'])-1:
